  ...
  try {
    ...
    if (path.startsWith('/api/admin/')) {
      ...
      if (path === '/api/admin/tasks/save' && request.method === 'POST') {
        ...
        if (id) {
          await dbRun(env, 'UPDATE tasks SET title=?,description=?,icon=?,task_type=?,url=?,ad_url=?,points_reward=?,timer_seconds=?,display_order=?,is_active=? WHERE id=?',
            [title, description||null, icon||'🎯', task_type, url||null, ad_url||null, points_reward, timer_seconds||0, display_order||99, is_active?1:0, id]);
        } else {
          await dbRun(env, "INSERT INTO tasks (id,title,description,icon,task_type,url,ad_url,points_reward,timer_seconds,display_order,is_active,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,datetime('now'))",
            [crypto.randomUUID(), title, description||null, icon||'🎯', task_type, url||null, ad_url||null, points_reward, timer_seconds||0, display_order||99, is_active?1:0]);
        }
        return json({ success: true });
      }
